...
export default function ForestEvents({participantInfo}){
    ...
    const leaveEvent = (id,partCount) => {
        ...
        const eventDetailsEditor = (id) => {
            Axios.put("http://localhost:5000/update/forest_events/details", {
                id: id,
                address: place,
                time: eventTime,
                date: eventDate,
        ...
        const eventDeleter = (id) => {
            Axios.delete(`http://localhost:5000/delete/forest_events/${id}`).then(() => {
                setForestEventData(forestEventData.filter(val => {
                    return id === val.id
                }))
            })
        }
    ...
    return(
        <>
        {participantInfo.Username === "AdminMherK" ? <AdminOnlyAccess lakeEventData={forestEventData} setlakeEventData={setForestEventData} eventName={"forestApi"}/> : ""}
            {
