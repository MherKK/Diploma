...
export default function LakeEvents({participantInfo}){
    ...
    let [lakeEventData,setlakeEventData] = useState([]);
    ...
    useEffect(() =>{
        Axios.get("http://localhost:5000/lakeApi").then(
            response => setlakeEventData(response.data)
            )
        },[lakeEventData.length])
    ...
    const  updateParticipant =  (id,value,partCount) => {   
        ...
        if(partCount === value.participantsLimit) {
            setlakeEventData(lakeEventData.map((val) => {
    ...
    const eventDetailsEditor = (id) => {
        Axios.put("http://localhost:5000/update/lake_events/details", {
            id: id,
            address: place,
            time: eventTime,
            date: eventDate,
    ...
    const eventDeleter = (id) => {
        Axios.delete(`http://localhost:5000/delete/lake_events/${id}`).then(() => {
            setlakeEventData(lakeEventData.filter(val => {
                return id === val.id
            }))
        })
    }
    ...
    return(
        <>
            {participantInfo.Username === "AdminMherK" ? <AdminOnlyAccess lakeEventData={lakeEventData} setlakeEventData={setlakeEventData} eventName={"lakeApi"}/> : ""}
            {
                    lakeEventData.map((Value ,index) => {
                        return(
